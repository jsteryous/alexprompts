...
export interface Prospect {
  ...
  notes: string | null;
  packet_html_url: string | null;
  packet_envelope_text: string | null;
  packet_generated_at: string | null;
  packet_emailed_at: string | null;
}
...
// ── Mailable packet (cover letter + audit findings, generated by
//    scripts/prospects/audit_packet.py --upload). Surfaces here as a "View
//    packet ↗" link so the user doesn't have to open VS Code to print one. ─

function PacketSection({ prospect }: { prospect: Prospect }) {
  const url = prospect.packet_html_url;
  const envelope = prospect.packet_envelope_text;
  const generatedAt = prospect.packet_generated_at;
  const emailedAt = prospect.packet_emailed_at;
  const [copied, setCopied] = useState(false);
  ...
  if (!url && !envelope) {
    return (
      <section className="space-y-2">
        ...
        <div className="theme-card-muted border theme-border rounded p-3 text-xs theme-text-muted">
          Not yet generated. Daily packet workflow will pick this up if it&rsquo;s
          in the top severity pool. Run locally with{" "}
          <code className="theme-text-secondary">
            python -m prospects.audit_packet --place-id {prospect.place_id ?? "<id>"} --upload
          </code>.
        </div>
      </section>
    );
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-baseline justify-between">
        ...
        {emailedAt && (
          <span className="text-xs tone-good-text">Sent to inbox</span>
        )}
      </div>
      <div className="theme-card-muted border theme-border rounded p-3 space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
        ...
        {envelope && (
          <div>
            <div className="flex items-baseline justify-between mb-1">
              <span className="text-xs theme-text-muted">Envelope</span>
              <button
                type="button"
                onClick={onCopyEnvelope}
                className="text-xs theme-text-muted hover:theme-text-primary underline decoration-dotted underline-offset-2"
              >
                {copied ? "Copied" : "Copy"}
              </button>
            </div>
            <pre className="text-xs theme-text-secondary theme-card border theme-border rounded p-3 whitespace-pre-wrap break-words">
              {envelope}
            </pre>
          </div>
        )}
    ...
    <>
      ...
      >
        <div className="p-6 space-y-6">
          ...
          {/* Mailable audit packet — generated by audit_packet.py, uploaded to
              prospect-audits Storage. "View packet ↗" opens the print-ready HTML. */}
          <PacketSection prospect={prospect} />
