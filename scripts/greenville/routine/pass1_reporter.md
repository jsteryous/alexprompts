...
STEP 3, GET A LEAD IMAGE (a cascade: a genuinely relevant Commons photo first, otherwise a render of the location). The image must be one anyone can reuse. NEVER scrape the publisher's photo or its `og:image`; news photos are copyrighted by the outlet or a wire service and are not ours to republish. Work the cascade in order and stop at the first option that yields a genuinely relevant image. The whole point is that the image actually relates to THIS story, so a generic stock photo is never acceptable. Because every real-estate story happens somewhere, a render of the location is the FLOOR: unless the story is genuinely placeless (option C), you ALWAYS have at least a map, so `none` should be vanishingly rare. A brand-new building that is not yet on Commons is a `map`, not a `none`.
  ...
  C. NONE, almost never. Choose this ONLY when the story is truly placeless: it has no building, development, neighborhood, corridor, road, intersection, or town you could point to on a map (for example a pure interest-rate or county-budget item with no site). If you named ANY address, building, road, development, or neighborhood anywhere in the facts, you have a geocodable LOCATION, so the answer is `map` (option B), NOT `none`. A brand-new building not yet on Wikimedia Commons is the normal case for `map`, never a reason to fall back to `none`. Set IMAGE = none only after you have confirmed there is no place to pin. The article then opens on text. A missing image is acceptable only when no location exists; a generic, wrong, or unlicensed one never is.
