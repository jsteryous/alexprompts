...
- INTERNAL LINKS (they help SEO, keep the reader on the site, and feed the referral funnel): where natural, link to the site's own stable pages with root-relative markdown links. The tools: the mortgage math to `/tools/mortgage`, a rental analysis to `/tools/deal-analyzer`, recent local commercial sales to `/tools/buyers-list`. The section hub `/real-estate` when you reference "more Greenville guides." And the referral connector `/find-a-pro` (also required in the close below). Two or three internal links, only where they genuinely help. Do not link to a specific other article by slug, because you cannot confirm it exists; link the stable hub or tool instead. Do not stuff them.
