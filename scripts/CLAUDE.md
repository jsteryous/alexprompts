...
- **`greenville/`** — the local Greenville engine; documented in `scripts/greenville/CLAUDE.md`.
  ...
  `greenville/cover_ingest.py`, the monthly GitHub Action that grew a curated cover library from
  Wikimedia Commons, was **deleted August 27, 2026** with the auto-cover; covers are Alex's own
  uploads now. See `scripts/greenville/CLAUDE.md`.
