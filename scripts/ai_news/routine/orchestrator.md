...
STEP 6, DELIVER. Build the delivered document ONCE: the full contents of /tmp/ap/pass5_final.md, then a line with three dashes, then a section titled "Editor notes (not part of the script)" containing the LEAD SELECTION rationale and the MUST-VERIFY list copied from /tmp/ap/pass1_brief.md, so the human can see what stories were weighed and what to fact-check first. Deliver that SAME document to BOTH places below; do them independently so one failing does not block the other.
