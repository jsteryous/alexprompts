...
Then pick the SPINE QUESTION: from QUESTIONS THE STORY RAISES, choose the ONE the whole script will carry. Prefer one that is OPEN, or one that resolves into a real reveal; never pick empty rhetoric.

Output:
- CHOSEN LENS: one line, plus the optional second and why it sharpens.
- SPINE QUESTION: one line, with its tag ANSWERED/OPEN/SPECULATIVE.
- WHY THIS ANGLE: 3 to 5 sentences grounded in the facts.
- WHAT TO CALIBRATE: believe strongly versus hold loosely.
- FACT VS SPECULATION: which load-bearing points are confirmed versus inference.
