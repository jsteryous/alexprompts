...
- Homepage hero eyebrow: "For dentists whose reputations are better than their websites." H1: "Your patients love you. Your website doesn't show it." City pages still carry the geo in their own hero copy; practice-type pages carry their specialty-specific hero. "Dental Website Cleanup" is still the wedge keyword but it moves down into PricingSection / FAQ / metadata rather than headlining the hero — StoryBrand positioning took priority over keyword anchoring on the homepage specifically. Cleanup-anchored eyebrows remain on city pages.
- **StoryBrand structure** (April 2026 rewrite): homepage is explicitly a StoryBrand arc — hero (dentist whose reputation is better than their website) → villain (StakesSection, "the invisible leak") → guide's authority (BeforeAfterSection) → plan (ProcessSection + CompetenceSection) → supporting evidence (HipaaSection, demoted) → victory (SuccessSection) → price → FAQ → lead magnet → final CTA. Homepage H1 is "Your patients love you. Your website doesn't show it." Eyebrow one-liner: "For dentists whose reputations are better than their websites." The villain is the *patient you never meet* — concrete, not abstract. The authority stat is the cohort cut (established practices ≥100 Google reviews with strong star averages + % scoring below 50 on mobile LH), not the raw portfolio median. HIPAA copy was demoted from fear-lever headliner to "boring stuff that protects you" supporting beat. See `src/components/HomeSections.tsx` for each section; `StakesSection` takes optional `cohortN / cohortAvgRating / cohortUnder50Pct` props for the live stat callout.
- **BeforeAfterSection** shows the three patterns we fix (Speed / Contact form / Mobile-first) above centered `theme-badge` pills. Homepage passes a live intro built from `fetchPortfolioStats()` — which now returns the established cohort (`established_n / established_avg_rating / established_lh_under_50_pct`) alongside the portfolio-wide stats. Falls back to the portfolio stat if cohort n < 10, and to hardcoded qualitative copy if the whole query fails. Practice-type pages use the generic default intro. City pages REPLACE the section with `<CityAuditStats>` when per-county data exists. Never fabricate named case studies.
- **CompetenceSection** renders on `/page.tsx` + `/dental-website-cleanup/[city]` only — practice-type pages dropped it as meta-redundant. `FaqSection` takes optional `{ items?, eyebrow?, headlineTop?, headlineBottom?, lede? }` so practice-type pages swap in specialty FAQs; pair with `buildFaqJsonLd(items)` to keep JSON-LD matching rendered content. `CompetenceSection` reads `practiceTypeList` and applies `p.imagePosition` (CSS `object-position`) when set; row labels render `text-sm md:text-base font-extrabold`. `HipaaSection` grid ratio is `1.4fr,0.6fr` (intentional). No `tiers` array; pricing is a single "starts at $1,500" anchor in `PricingSection`. Body CTAs across homepage/city/practice-type pages all read "Get your free audit" — do not reintroduce "Show me what's broken" (pre-StoryBrand framing, implies we pre-judge). Nav CTA stays Title-case "Get Free Audit" as a button label.
...
- **`src/components/HomeSections.tsx`** — shared marketing sections: `HipaaSection`, `ProcessSection`, `CompetenceSection`, `BeforeAfterSection`, `StakesSection`, `PricingSection`, `FaqSection`, `FinalCtaSection` + `faqs`, `faqJsonLd`, `buildFaqJsonLd(list)`, `Faq`, `ArrowIcon`. `CompetenceSection` renders on `/page.tsx` + `/dental-website-cleanup/[city]` only — practice-type pages dropped it as meta-redundant. `FaqSection` takes optional `{ items?, eyebrow?, headlineTop?, headlineBottom?, lede? }` so practice-type pages swap in specialty FAQs; pair with `buildFaqJsonLd(items)` to keep JSON-LD matching rendered content. `CompetenceSection` reads `practiceTypeList` and applies `p.imagePosition` (CSS `object-position`) when set; row labels render `text-sm md:text-base font-extrabold`. `HipaaSection` grid ratio is `1.4fr,0.6fr` (intentional). `BeforeAfterSection` accepts `{ intro?, methodology?, patterns? }` — homepage (`src/app/page.tsx`, async, `revalidate = 3600`) wires it to `fetchPortfolioStats()` for live n / Lighthouse median / %-under-50 across all 5 counties with a matching methodology footer; falls back to hardcoded `87 / 55 / 39%` copy when <10 audited rows or the query fails. City pages REPLACE the whole section with `<CityAuditStats>` when per-county data exists. No `tiers` array; pricing is a single "starts at $1,500" anchor in `PricingSection`.
