...
- Homepage hero eyebrow: "Dental Website Cleanup & Audits" (softened from the earlier location-anchored "Dental Website Cleanup · Greenville SC" — city pages still carry the geo). H1 is softened to "might be costing you patients" for credibility.
- Three load-bearing homepage beats: **StakesSection** reframes the problem ("you don't have a traffic problem — you have a booking problem"); **HipaaSection** is the fear lever (PHI exposure, BAA-backed intake) with a two-column layout — tightened copy + `/public/hipaa.jpg`; **CompetenceSection** is the competence lever — a 5-row practice-type matrix (emergency / cosmetic / pediatric / sedation / FFS-membership) with per-row photography. **BeforeAfterSection** carries proof-of-inspection stats from `audit_stats.py` (n=87, median mobile Lighthouse 55, 39% below 50) above three problem→fix→result cards whose labels render as centered bold `theme-badge` pills (Speed / Contact form / Mobile-first experience). Never fabricate named case studies.
...
- **`src/components/HomeSections.tsx`** — exports the shared marketing sections consumed by `/page.tsx`, `/dental-website-cleanup/[city]/page.tsx`, `/[practiceType]/page.tsx`, and `/sample-proposal/page.tsx`: `HipaaSection`, `ProcessSection`, `CompetenceSection`, `BeforeAfterSection`, `StakesSection`, `PricingSection`, `FaqSection`, `FinalCtaSection`, plus `faqs`, `faqJsonLd`, `ArrowIcon`. No `tiers` array — pricing is a single "starts at $1,500" anchor rendered inline in `PricingSection`; larger scope lives in the written proposal only. `CompetenceSection` reads from `src/lib/practiceTypes.ts` (`practiceTypeList`) — each row renders a "How we build for X practices" link to `/{slug}`. Row labels are rendered `text-sm md:text-base font-extrabold` (intentional upsize — earlier `text-xs font-semibold` disappeared next to the photography). `HipaaSection` pulls `/public/hipaa.jpg` in a narrower right column (grid ratio `1.4fr,0.6fr` — the earlier `1.1fr,0.9fr` made the photo overwhelm the copy). `BeforeAfterSection` is driven by `fixPatterns` (labels render as `theme-badge` pills, centered) and has a hardcoded stat block — update the `n=87 / 55 / 39%` numbers when `python scripts/audit_stats.py` shifts materially.
- **`src/lib/practiceTypes.ts`** — `PracticeTypeSlug` union (`cosmetic-dentist-website`, `pediatric-dentist-website`, `sedation-dentist-website`, `emergency-dentist-website`, `fee-for-service-dentist-website`). Each entry carries label + CompetenceSection content (image, alt, headline, bullets) + per-page marketing (heroEyebrow, heroH1Top/Bottom, heroLede, heroBody) + SEO (metaTitle, metaDescription, serviceName, serviceDescription). Adding a practice type = edit this file; the dynamic route, sitemap, footer, and CompetenceSection pick it up automatically. Slugs ARE the URLs (flat top-level, keyword-first) — renaming breaks inbound links.
- **`src/app/[practiceType]/page.tsx`** — dynamic top-level route with `dynamicParams = false` + `generateStaticParams` from `practiceTypeSlugs`. Renders a hero-per-practice plus every shared marketing section. Per-page `Service` + `Offer` ($1,500) JSON-LD, areaServed = Upstate SC. Static Next.js routes at root take precedence, so this catches only the five practice-type slugs — but don't add a root-level static route whose name happens to match a practice-type slug, and don't rename slugs without a 301 plan.
