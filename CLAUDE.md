...
| `ANTHROPIC_API_KEY` | Repo **GitHub Actions secret** for the monthly Greenville cover-library grower (`scripts/greenville/cover_ingest.py`, run from `.github/workflows/greenville-covers.yml`). Powers the Claude **vision** gate (Haiku) that scores candidate Wikimedia Commons photos before they enter the library. Not used by the site at runtime. Unset = the covers workflow's vision step fails (run it with `--no-vision`, or just skip the workflow); the curated library and everything else keep working. |
