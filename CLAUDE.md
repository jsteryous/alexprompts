...
- **`src/components/HomeSections.tsx`** — shared marketing sections: `HipaaSection`, `ProcessSection`, `CompetenceSection`, `BeforeAfterSection`, `StakesSection`, `PricingSection`, `FaqSection`, `FinalCtaSection` + `faqs`, `faqJsonLd`, `buildFaqJsonLd(list)`, `Faq`, `ArrowIcon`. `CompetenceSection` renders on `/page.tsx` + `/dental-website-cleanup/[city]` only — practice-type pages dropped it as meta-redundant. `FaqSection` takes optional `{ items?, eyebrow?, headlineTop?, headlineBottom?, lede? }` so practice-type pages swap in specialty FAQs; pair with `buildFaqJsonLd(items)` to keep JSON-LD matching rendered content. `CompetenceSection` reads `practiceTypeList` and applies `p.imagePosition` (CSS `object-position`) when set; row labels render `text-sm md:text-base font-extrabold`. `HipaaSection` grid ratio is `1.4fr,0.6fr` (intentional). `BeforeAfterSection` has a hardcoded n=87 / 55 / 39% stat block — city pages REPLACE it with `<CityAuditStats>` when per-county data exists. No `tiers` array; pricing is a single "starts at $1,500" anchor in `PricingSection`.
...
- **`src/lib/cityStats.ts` + `src/components/CityAuditStats.tsx`** — per-city first-party audit stats. `fetchCityStats(county)` reads `website_prospects` with the service key, scoped by `ilike("county", ...)`, returns `null` when fewer than 3 audited rows (too thin to cite). `<CityAuditStats>` is a NON-async component that takes pre-fetched stats as a prop — do not re-query inside it. Renders 4 stat tiles + up to 3 threshold-gated takeaways (each line only fires when its underlying stat crosses a threshold) + a methodology footer. On `/dental-website-cleanup/[city]/page.tsx` this REPLACES `<BeforeAfterSection />` when `fetchCityStats` returns a value; falls back to the generic section otherwise. City page is `revalidate = 3600`.
